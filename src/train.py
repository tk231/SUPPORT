...
if __name__ == "__main__":
    ...
    os.makedirs(f"{opt.results_dir}/images/{opt.exp_name}", exist_ok=True)
    os.makedirs(f"{opt.results_dir}/saved_models/{opt.exp_name}", exist_ok=True)
    os.makedirs(f"{opt.results_dir}/logs", exist_ok=True)
    logging.basicConfig(level=logging.INFO, filename=f"{opt.results_dir}/logs/{opt.exp_name}.log",
                        filemode="a", format="%(name)s - %(levelname)s - %(message)s")
    writer = SummaryWriter(f"{opt.results_dir}/tsboard/{opt.exp_name}")
    ...
    if opt.epoch != 0:
        model.load_state_dict(
            torch.load(f"{opt.results_dir}/saved_models/{opt.exp_name}/model_{opt.epoch - 1}.pth"))
        optimizer.load_state_dict(
            torch.load(f"{opt.results_dir}/saved_models/{opt.exp_name}/optimizer_{opt.epoch - 1}.pth"))
        print(f'Loaded pre-trained model and optimizer weights of epoch {opt.epoch - 1}')
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        ...
        if (opt.checkpoint_interval != -1) and (epoch % opt.checkpoint_interval == 0):
            torch.save(model.state_dict(), f"{opt.results_dir}/saved_models/{opt.exp_name}/model_{epoch}.pth")
            torch.save(optimizer.state_dict(), f"{opt.results_dir}/saved_models/{opt.exp_name}/optimizer_{epoch}.pth")

...
if __name__ == '__main__':
    ...
    model = SUPPORT(in_channels=61, mid_channels=[16, 32, 64, 128, 256], depth=5, blind_conv_channels=64,
                    one_by_one_channels=[32, 16], last_layer_channels=[64, 32, 16],
                    bs_size=bs_size).cuda()

    model.load_state_dict(torch.load(model_file))

    list_of_volumes_to_denoise = glob.glob(f"{data_folder}/*.tif")
    # list_of_volumes_to_denoise = glob.glob(f"{data_folder}/*.tiff")

    for img_file in list_of_volumes_to_denoise:
        # demo_tif = torch.from_numpy(skio.imread(data_file).astype(np.float32)).type(torch.FloatTensor)
        demo_tif = torch.from_numpy(skio.imread(img_file).astype(np.float32)).type(torch.FloatTensor)
        demo_tif = demo_tif[:, :, :]
        ...
        # Create savepath for file
        file_basename = os.path.basename(img_file).split('.')[0]
        output_designator = "_denoised"
        file_ext = '.tif'
        output_filepath = os.path.join(output_folder, f"{file_basename}{output_designator}{file_ext}")

        ##################################################

        # skio.imsave(output_filepath, denoised_stack[(model.in_channels - 1) // 2:-(model.in_channels - 1) // 2, :, :],
        #             metadata={'axes': 'TYX'})
        tifffile.imwrite(output_filepath, denoised_stack[(model.in_channels - 1) // 2:-(model.in_channels - 1) // 2, :, :], photometric='minisblack', metadata={'axes': 'ZYX'})
